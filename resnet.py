...
class ResNet50(nn.Module):
    def __init__(
        ...
        # Standard ResNet stem for 224x224 inputs.
        ...
        self.fc = nn.Linear(widths[3] * Skeleton.expansion, num_classes)
    ...
    def _init_weights(self) -> None:
        # He et al. initialization (Kaiming) for ReLU-based conv nets.
    ...
    def forward_proba(self, x: torch.Tensor) -> torch.Tensor:
        return torch.softmax(self.forward_logits(x), dim=1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Convert zero-based index [0..num_classes-1] to class ID [1..num_classes].
...
def build_training_components(
    ...
    - Optimizer: Adam
    - LR: 0.1
    - Reduce LR by factor 10 when validation error plateaus
    - Weight decay: 1e-5
    - No dropout
    """
